...
run_metropolis_MCMC = function(startvalue, iter){
...
parameter_mean_estimation = function(chain, burnin=0.5, dropout=100){
...
K = block2$doses
# lambda = 2 
lambda  = find_optimal_lambda(x,y,K, iter = 100, lambdas = c(0.01, 0.1, 2, 5))
chain = chain_NPB(x,y,K,lambda, iter = 1000)
make_plots(chain, K, lambda, logplotx=T, title='dir_fit')

param_est = parameter_mean_estimation(chain, burnin = 0.5, dropout = 10)
C_est = param_est[[1]]
sigma2_est = param_est[[2]]
a_est = param_est[[3]]
x_est = param_est[[4]]
